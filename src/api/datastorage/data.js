...
exports.createTableStatements = {
  ...
  studySubject: 'CREATE TABLE study_subject(pk_study_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, study_name VARCHAR(255) NOT NULL, fk_faculty_name VARCHAR(255), FOREIGN KEY (fk_faculty_name) REFERENCES faculty(pk_faculty_name))',
  user: 'CREATE TABLE user(pk_user_id VARCHAR(20) PRIMARY KEY NOT NULL, first_name VARCHAR(255), last_name VARCHAR(255), e_mail VARCHAR(255), last_action DATETIME)',
  history: 'CREATE TABLE history(pk_history_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, fk_user_id INTEGER, searched_rz_nr INTEGER, name VARCHAR(255), e_mail VARCHAR(255), faculty VARCHAR(255), subject VARCHAR(255), server_group VARCHAR(255), gender VARCHAR(255), date_entry DATETIME, FOREIGN KEY (fk_user_id) REFERENCES user(pk_user_id))',
  faculty: 'CREATE TABLE faculty(pk_faculty_name VARCHAR(255) PRIMARY KEY, university_name VARCHAR(255))',
