...
exports.createTableStatements = {
  ...
  user: 'CREATE TABLE user(pk_user_id VARCHAR(20) PRIMARY KEY NOT NULL, first_name VARCHAR(255), last_name VARCHAR(255), e_mail VARCHAR(255))',
  history: 'CREATE TABLE history(pk_history_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, fk_user_id INTEGER, searched_rz_nr INTEGER, name VARCHAR(255), e_mail VARCHAR(255), faculty VARCHAR(255), subject VARCHAR(255), server_group VARCHAR(255), gender VARCHAR(255), FOREIGN KEY (fk_user_id) REFERENCES user(pk_user_id))',
